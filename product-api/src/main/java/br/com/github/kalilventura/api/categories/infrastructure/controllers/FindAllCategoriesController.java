...
public final class FindAllCategoriesController {
  ...
  public @ResponseBody ResponseEntity<List<CategoryResponse>> get() {
    ...
    final var listeners =
        new FindAllCategoriesCommand.Listeners(onSuccess(wrapper), onEmpty(wrapper));
    command.execute(listeners);
    return wrapper.getResponse();
  }

  private Consumer<List<Category>> onSuccess(
      final ResponseHolder<List<CategoryResponse>> response) {
    return categories -> {
